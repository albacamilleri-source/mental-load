...
function queueMealPayload(item, week, day) {
  return {
    ...
    ingredients: item.ingredients, method: item.method || '', extracted_at: item.extracted_at, rating: item.rating, notes: item.notes || '',
...
function RecipeImporter({ categories, onImport, onManual, busy }) {
  ...
  return <form className="importForm" onSubmit={e => { e.preventDefault(); importTo('queue'); }}>
    ...
    <div className="importRow"><input className="input" aria-label="Recipe URL to import" type="url" value={url} disabled={busy} required placeholder="https://…" onChange={e => setUrl(e.target.value)}/><div className="importActions"><button type="button" className="btn secondary" disabled={busy || !url.trim()} onClick={() => importTo('library')}>{busy ? 'Importing…' : 'Save to library'}</button><button className="btn" disabled={busy || !url.trim()}>{busy ? 'Importing…' : 'Import & queue'}</button></div></div>
    {error && <div className="saveError" role="alert">{error}</div>}
    <div className="hint">Queue choices follow your day categories: {categories.filter(c => c.accepted_tags?.length).map(c => `Day ${c.day_number} · ${c.name || c.accepted_tags.join(' or ')}`).join('; ') || 'all days currently accept any recipe'}.</div>
  </form>;
}

function ManualRecipeModal({ busy, onClose, onSave }) {
  ...
  async function save(destination) {
    ...
    const message = await onSave({ title: title.trim(), source_ref: source.trim(), ingredients, method: method.trim(), tags: parseTags(tagsText) }, destination);
  ...
  return <div className="modalBack" role="dialog" aria-modal="true" aria-label="Add a recipe manually" onMouseDown={event => { if (event.target === event.currentTarget && !busy) onClose(); }}><div className="modal recipeDetailsModal">
    ...
    {error && <div className="saveError" role="alert">{error}</div>}
    <div className="actions"><button type="button" className="btn secondary" disabled={busy} onClick={onClose}>Cancel</button><button type="button" className="btn secondary" disabled={busy} onClick={() => save('library')}>{busy ? 'Saving…' : 'Save to library'}</button><button type="button" className="btn" disabled={busy} onClick={() => save('queue')}>{busy ? 'Saving…' : 'Save & queue'}</button></div>
  </div></div>;
}

function QueueRecipe({ item, index, count, tags, categories, busy, onTagsSaved, onMove, onBump }) {
...
function QueueManager({ queue, categories, tagMap, busy, onClose, onTagsSaved, onMove, onBump }) {
  return <div className="modalBack" role="dialog" aria-modal="true" aria-label="Manage recipe queues"><div className="modal queueModal">
    ...
    <div className="queueColumns">{categories.map(category => { const rows = queueForDay(queue, category.day_number); return <section className="queueDay" key={category.day_number}>
      <h3>Day {category.day_number}{category.name ? ` · ${category.name}` : ''}</h3><div className="small">{category.accepted_tags?.length ? `Accepts ${category.accepted_tags.join(' or ')}` : 'No required tag'}</div>
      {rows.length ? rows.map((item, index) => <QueueRecipe key={item.id} item={item} index={index} count={rows.length} tags={recipeTagsFor(item, tagMap)} categories={categories} busy={busy} onTagsSaved={onTagsSaved} onMove={onMove} onBump={onBump}/>) : <div className="empty queueEmpty">Queue empty</div>}
    </section>; })}</div>
  </div></div>;
}
...
function SwitchMealModal({ day, library, tagMap, busy, onClose, onSave }) {
  ...
  async function submit(e) {
    e.preventDefault();
    let recipe = { title: title.trim(), source_ref: source.trim(), ingredients: null, method: '', extracted_at: null, rating: null, notes: '' };
...
function RecipeDetailsModal({ recipe, busy, onClose, onSave }) {
  ...
  async function submit(event) {
    ...
    if (!source.trim()) { setError('Add a recipe URL or source.'); return; }
    const message = await onSave(recipe, { source_ref: source.trim(), method });
    if (message) setError(message);
  }
  return <div className="modalBack" role="dialog" aria-modal="true" aria-label={`Recipe details for ${recipe.title}`} onMouseDown={event => { if (event.target === event.currentTarget && !busy) onClose(); }}><form className="modal recipeDetailsModal" onSubmit={submit}>
    <div className="modalHead"><div><h2 className="sectionTitle">{recipe.title}</h2><div className="small">Recipe details</div></div><button type="button" className="iconBtn" aria-label="Close recipe details" disabled={busy} onClick={onClose}>×</button></div>
    ...
    {error && <div className="saveError" role="alert">{error}</div>}
    <div className="actions"><button type="button" className="btn secondary" disabled={busy} onClick={onClose}>Cancel</button><button className="btn" disabled={busy || !changed}>{busy ? 'Saving…' : 'Save changes'}</button></div>
  </form></div>;
}

function RecipeCard({ meal, tags, categories, meals, scheduledDays = [], queuedDays = [], onTagsSaved, onOpenDetails, onUse, onQueue, onDelete, busy, draggable = false, onDragStart, onDragEnd }) {
  const [draft, setDraft] = useState(tags.join(', '));
  ...
  const available = matchingDays(draftTags, categories).filter(c => !meals[c.day_number - 1].title.trim() && !meals[c.day_number - 1].source_ref.trim());
  ...
  return <div className={`recipeCard${draggable ? ' draggableRecipe' : ''}`} draggable={draggable && !busy && !changed} onDragStart={e => onDragStart?.(e, meal)} onDragEnd={onDragEnd}>
    <div className="recipeCardHead"><div><h4>{meal.title}</h4><div className="small">{meal.source_ref}</div></div><div className="recipeCardMeta"><span className={`recipeStatus ${meal.has_been_cooked ? 'isCooked' : ''}`}>{meal.has_been_cooked ? '✓ Cooked' : 'Not cooked yet'}</span>{scheduledDays.length > 0 && <span className="recipeStatus isScheduled">Scheduled · {scheduledDays.length === 1 ? 'Day' : 'Days'} {scheduledDays.join(', ')}</span>}{queuedDays.length > 0 && <span className="recipeStatus isQueued">Queued · {queuedDays.length === 1 ? 'Day' : 'Days'} {queuedDays.join(', ')}</span>}<button type="button" className="recipeExpand" onClick={() => onOpenDetails(meal)}>View recipe</button>{draggable && <span className="dragHint" aria-hidden="true">Drag to a day</span>}</div></div>
    ...
    <div className="dayActions">
      <span className={`tagSaveState ${saveState}`}>{saveState === 'pending' ? 'Saving soon…' : saveState === 'saving' ? 'Saving…' : saveState === 'saved' ? '✓ Tags saved' : saveState === 'error' ? 'Save failed' : 'Tags save automatically'}</span>
      ...
      <button className="btn" disabled={busy || !selected || tagsSaving} onClick={async () => setError(await onUse(meal, Number(selected)) || '')}>Use recipe</button>
...
export default function MealPlanner({ onDirtyChange } = {}) {
  ...
  async function saveRecipeDetails(recipe, details) {
    ...
    try {
      ...
      const payload = {
        ...
        method: String(details.method || '').trim(), extracted_at: recipe.extracted_at, rating: recipe.rating, notes: recipe.notes || '',
      ...
      setQueueRows(prev => prev.map(row => recipeKey(row) === oldKey ? { ...row, source_ref: source, method: payload.method } : row));
  ...
  async function saveManualRecipe(recipe, destination) {
    ...
    try {
      ...
      const payload = {
        recipe_key: key, title: recipe.title, source_ref: recipe.source_ref, ingredients: recipe.ingredients, method: recipe.method || '',
      ...
      if (destination === 'queue') {
        ...
        if (!alreadyQueued) {
          const day = chooseQueueDay(recipe.tags, categories, meals);
          const queuePayload = {
          ...
          setToast(`${recipe.title} added to Day ${day} queue`);
  ...
  async function queueLibraryRecipe(recipe) {
    ...
    try {
      ...
      setQueueRows(nextQueue); setGroceryGenerated(false); setToast(`${recipe.title} added to Day ${day} queue`);
      return '';
  ...
  const libraryRecipes = useMemo(() => {
    ...
    [...pastRows, ...queueRows, ...savedMeals, ...libraryRecords.filter(recipe => !recipe.is_deleted)].forEach(recipe => {
      ...
      recipes.set(key, previous ? {
        ...
        has_been_cooked: !!(recipe.has_been_cooked || previous.has_been_cooked),
        recipe_key: recipe.recipe_key || previous.recipe_key || key,
      } : { ...recipe, method: recipe.method || '', recipe_key: recipe.recipe_key || key });
    });
  ...
  return <div className="meal-planner"><div className="app"><div className="shell">
    ...
    <section className="card">
      ...
      {loadingWeek ? <div className="empty">Loading week…</div> : !loadError && meals.map((meal, index) => {
        ...
        return <details className={`meal${canDrop ? ' dropReady' : ''}${dropTarget === meal.meal_number ? ' dropActive' : ''}`} key={meal.meal_number}
          ...
          <summary className="mealSummary"><div><h3 className="dayHeading">Day {meal.meal_number}</h3><div className="dayCategory">{meal.is_override ? `Temporary ${meal.override_type || 'manual'} switch` : category?.name || 'Any recipe'}</div></div><div className={`mealSummaryValue${meal.title.trim() ? '' : ' isEmpty'}`}>{meal.title.trim() || 'Empty'}<span className="mealChevron" aria-hidden="true">⌄</span></div></summary>
    ...
    {detailsRecipe && <RecipeDetailsModal recipe={detailsRecipe} busy={busy} onClose={() => setDetailsRecipe(null)} onSave={saveRecipeDetails}/>}
